...
type IncidentFilterSheetProps = {
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  incidentTypesWithCounts: { type: string; count: number }[];
  selectedTypes: string[];
  onApply: (selectedTypes: string[]) => void;
};

export function IncidentFilterSheet({
  isOpen,
  onOpenChange,
  incidentTypesWithCounts,
  selectedTypes,
  onApply,
}: IncidentFilterSheetProps) {
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent className="flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>
            Seleccione uno o más tipos de incidentes para filtrar la lista.
          </SheetDescription>
        </SheetHeader>
        <ScrollArea className="flex-grow my-4">
          <div className="space-y-3 pr-6">
            {incidentTypesWithCounts.map(({ type, count }) => (
