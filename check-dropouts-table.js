...
async function checkDropoutsTable() {
  console.log('🔍 Checking dropouts table structure...');
  
  try {
    // Try to get table columns using RPC
    console.log('\n1. Trying to get table columns using RPC...');
    const { data: rpcData, error: rpcError } = await supabase
      .rpc('get_table_columns', { table_name: 'dropouts' });
    ...
    const testColumns = [
      ...
      ['id', 'student_id', 'dropout_date', 'reason', 'notes']
    ];
    ...
    if (allError) {
      ...
      if (allData && allData.length > 0) {
        ...
        const testRecord = {
          ...
          reported_by: '00000000-0000-0000-0000-000000000001' // dummy UUID
        };
...
checkDropoutsTable().then(() => {
  console.log('\n✅ Dropouts table check completed');
  process.exit(0);
}).catch(error => {
