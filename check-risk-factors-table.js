...
async function checkRiskFactorsTable() {
  console.log('🔍 Checking risk_factors table structure...');
  
  try {
    // Try to get table columns using RPC
    console.log('\n1. Trying to get table columns using RPC...');
    const { data: rpcData, error: rpcError } = await supabase
      .rpc('get_table_columns', { table_name: 'risk_factors' });
    ...
    const testColumns = [
      ...
      ['id', 'student_id', 'date'],
      ['id', 'student_id', 'risk_level'],
      ['id', 'student_id', 'risk_category'],
      ['id', 'student_id', 'category', 'level', 'notes']
    ];
    ...
    if (allError) {
      ...
      if (allData && allData.length > 0) {
        ...
        const testRecord = {
          ...
          severity: 'Medio',
          identified_by: '00000000-0000-0000-0000-000000000001' // dummy UUID
        };
...
checkRiskFactorsTable().then(() => {
  console.log('\n✅ Risk factors table check completed');
  process.exit(0);
}).catch(error => {
